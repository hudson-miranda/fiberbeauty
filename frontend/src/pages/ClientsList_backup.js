...
const ClientCard = ({ client, onEdit, onDelete, onView, onStartAttendance, onEndAttendance }) => {
  ...
  return (
    <Card className="hover:shadow-lg transition-shadow dark:bg-gray-800 h-full min-h-[280px] flex flex-col">
...
const ClientsList = () => {
  ...
  const { gridClasses, isMobile, isTablet } = useResponsiveGrid();
  ...
  const handleNPSSuccess = () => {
    ...
    fetchClients(currentPage, debouncedSearch, filters); // Recarregar para refletir mudanças
  };
  ...
  const hasActiveFilters = Object.entries(filters)
    .filter(([key]) => key !== 'includeInactive') // Exclui includeInactive da contagem
    .some(([, value]) => value !== '');
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          {viewMode === 'cards' ? (
            <div className={`mt-6 ${gridClasses} gap-4 sm:gap-6`}>
              {clients.map(client => (
                <ClientCard
                  key={client.id}
                  client={client}
